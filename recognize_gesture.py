...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'#屏蔽通知信息，警告信息，和错误信息，输出FATAL
tf.compat.v1.logging.set_verbosity(tf.compat.v1.logging.ERROR)#让tensorflow只将错误信息进行记录
classifier = tf.estimator.Estimator(model_dir="tmp/cnn_model2", model_fn=cnn_model_fn)
'''Estimator对象包装由一个名为model_fn函数指定的模型，model_fn在给定输入和许多其他参数的情
况下，返回执行训练、评估或预测所需的操作。所有输出（checkpoints, event files, etc.等）都写入
model_dir或其子目录'''

prediction = None
model = load_model('cnn_model_keras2.h5')#加载模型

def get_image_size():#获取图像的尺寸
...
def keras_process_image(img):#对图像进行处理
	img = cv2.resize(img, (image_x, image_y))#改变图像大小
	img = np.array(img, dtype=np.float32)#指定数据类型
...
keras_predict(model, np.zeros((50, 50), dtype=np.uint8))#np.zeros((50, 50)返回来一个50*50用0填充的数组
recognize()
